...
async function apiFetch(path, options = {}, isRetry = false) {
  ...
  if (res.status === 429 && !isRetry) {
    // Spotify never sets Access-Control-Expose-Headers, so Retry-After is
    // structurally unreadable from browser JS on this cross-origin request
    // (visible in devtools' Network tab, but not via fetch()'s Headers) —
    // there's no client-only way to show the real retry time here.
